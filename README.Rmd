...
#### As an array
Retrieving data will return contiguous arrays of data. We provide a function `get_var_array` that expects the slab coordinates in integer run-length encoding for each dimension as \code{[start_index, count]}. This function returns an array of cell values.
...
#### As a stars object
The `get_var` function is a convenient wrapper around the `get_var_array` function.  Users specify the slab limits with 'real world' units, and a [`stars`](https://r-spatial.github.io/stars/) is returned (although returning an array is an option).
